...
const ModificarPersonaje = () => {
    ...
    const onSubmit = async (data) => {
        ...
        if (data.alternate_names.length > 0) data.alternate_names = data.alternate_names.split(',').map(s => s.trim())
        if (data.alternate_actors.length > 0) data.alternate_actors = data.alternate_actors.split(',').map(s => s.trim())
        if (data.wand) {
        ...
        // try {
        //     await updatePersonajes(data, id)
        //     navigate('/')
        // } catch (e) {
        //     console.error(e)
        // }
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="container mt-4" style={{ maxWidth: "600px" }}>
            ...
            <div className="mb-3">
                ...
                <input type="text" id="alternate_names" className="form-control" {...register("alternate_names")}
                    defaultValue={initialData?.alternate_names ? [...initialData?.alternate_names]?.join(', ') : ""} />
            </div>
            {/* Species */}
            <div className="mb-3">
            ...
            <div className="mb-3">
                ...
                <input type="text" id="alternate_actors" className="form-control" {...register("alternate_actors")} defaultValue={initialData?.alternate_actors ? initialData?.alternate_actors?.join(', ') : ""} />
            </div>
            {/* Alive */}
            <div className="form-check mb-3">
            ...
            <div className="mb-3">
                <div>
                    {initialData?.image && <img src={initialData?.image?.includes("imagekit") ? initialData?.image : `http://localhost:2026/uploads/${initialData?.image}`} alt="" />}
                </div>
                <label htmlFor="image" className="form-label">Reemplazar imagen</label>
